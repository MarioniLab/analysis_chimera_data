...
This script is the R code used to create figure panels for Figure 1. 
...
Plot atlas labelled by cell type
...
Plot nearest neighbours of chimera cells in atlas, first for WT.
...
Plotting the expression of T and Mixl for atlas cells

```{r atlas_T_expression, dev='png'}
sce_temp <- cbind(atlas_sce[,atlas_sce$T_expression==0],
                  atlas_sce[,atlas_sce$T_expression>0])
p_T_expression <- plotReducedDim(sce_temp,colour_by = "T_expression",dimred = "umap",point_size=1) + theme(legend.position = "bottom",legend.title=element_blank(),axis.text.x=element_blank(),
      axis.ticks.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks.y=element_blank()) + scale_color_viridis_c()
print(p_T_expression)
```

```{r atlas_T_expression_legend,dev='svg',dev='pdf'}
legend_T_expression <- get_legend(p_T_expression)
as_ggplot(legend_T_expression)
```

```{r atlas_Mixl1_expression, dev='png'}
sce_temp <- cbind(atlas_sce[,atlas_sce$Mixl1_expression==0],
                  atlas_sce[,atlas_sce$Mixl1_expression>0])
p_Mixl1_expression <- plotReducedDim(sce_temp,colour_by = "Mixl1_expression",dimred = "umap",point_size=1) + theme(legend.position = "bottom",legend.title=element_blank(),axis.text.x=element_blank(),
      axis.ticks.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks.y=element_blank()) + scale_color_viridis_c()
print(p_Mixl1_expression)
```

```{r atlas_Mixl1_expression_legend,dev='pdf'}
legend_Mixl1_expression <- get_legend(p_Mixl1_expression)
as_ggplot(legend_Mixl1_expression)
```
